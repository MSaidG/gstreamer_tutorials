...
void add_stream_to_gl_mixer(GstElement *pipeline, GstElement *mixer,
  ...
  GstElement *filesrc, *demuxer, *parser, *decoder, *glupload, *glconvert, *queue;
  ...
  if (!filesrc || !demuxer || !parser || !decoder || !glupload || !glconvert || !queue) {
  ...
  g_object_set(mixer_sink_pad, "xpos", xpos, "ypos", ypos, "width", width, "height", height, NULL);
...
int main(int argc, char *argv[]) {
  ...
  GstElement *gl_convert_final = gst_element_factory_make("glcolorconvert", "final-gl-conv");
  ...
  GstElement *capsfilter = gst_element_factory_make("capsfilter", "format-fixer");

  // 4. Sink
  GstElement *sink = gst_element_factory_make("kmssink", "sink");

  if (!pipeline || !mixer || !gl_convert_final || !download || !capsfilter || !sink) {
  ...
  // but we restrict formats to what typical GPU/KMS combos handle well (RGBA/BGRA)
  GstCaps *caps = gst_caps_from_string("video/x-raw, format={ (string)BGRA, (string)RGBA, (string)BGRx }");
  g_object_set(capsfilter, "caps", caps, NULL);
  gst_caps_unref(caps);

  gst_bin_add_many(GST_BIN(pipeline), mixer, gl_convert_final, download, capsfilter, sink, NULL);

  // Link: Mixer -> GLConvert -> Download -> CapsFilter -> KMSSink
  if (!gst_element_link_many(mixer, gl_convert_final, download, capsfilter, sink, NULL)) {
  ...
  add_stream_to_gl_mixer(pipeline, mixer, "videos/earth.mp4", 960, 0, 960, 540);
  add_stream_to_gl_mixer(pipeline, mixer, "videos/ocean.mp4", 0, 540, 960, 540);
  add_stream_to_gl_mixer(pipeline, mixer, "videos/galaxy.mp4", 960, 540, 960, 540);
