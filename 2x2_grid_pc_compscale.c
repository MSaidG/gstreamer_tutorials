...
void add_stream_to_compositor(GstElement *pipeline, GstElement *compositor,
  ...
  GstElement *filesrc, *demuxer, *parser, *decoder, *queue;
  ...
  if (!filesrc || !demuxer || !parser || !decoder || !queue) {
    g_printerr("Not all elements could be created for file: %s\n", filename);
    return;
  }

  g_object_set(filesrc, "location", filename, NULL);
  gst_bin_add_many(GST_BIN(pipeline), filesrc, demuxer, parser, decoder, queue,
                   NULL);

  gst_element_link(filesrc, demuxer);
  gst_element_link_many(parser, decoder, queue, NULL);
  ...
  g_object_set(comp_sink_pad, "xpos", xpos, "ypos", ypos, "width", 960,
               "height", 540, NULL);
